...
double get_item_value(void *, int);
...
void ascii_get_element(PlyFile *, void *);
void binary_get_element(PlyFile *, void *);
...
void ply_put_element(PlyFile *plyfile, void *elem_ptr)
{
	...
	elem_data = elem_ptr;
	other_ptr = reinterpret_cast<char *>(elem_ptr) + elem->other_offset;
	...
	if (plyfile->file_type == PLY_ASCII) {
		...
		for (j = 0; j < elem->nprops; j++) {
			...
			if (elem->store_prop[j] == OTHER_PROP)
				memcpy(&elem_data, other_ptr, sizeof(void *));
			else
				memcpy(&elem_data, &elem_ptr, sizeof(void *));
			if (prop->is_list) {
				item = reinterpret_cast<char *>(elem_data) + prop->count_offset;
				...
				memcpy(&item, item_ptr, sizeof(void *));
				...
				for (k = 0; k < list_count; k++) {
					...
					write_ascii_item (fp, int_val, uint_val, double_val,
						prop->external_type);
					item = reinterpret_cast<char *>(item) + item_size;
				}
			}
			else {
				item = reinterpret_cast<char *>(elem_data) + prop->offset;
...
//		  return;
  ...
  ******************************************************************************/
  ...
  {
	  if (plyfile->file_type == PLY_ASCII)
		  ascii_get_element (plyfile, elem_ptr);
	  else
		  binary_get_element (plyfile, elem_ptr);
  }
  ...
  ******************************************************************************/
  ...
  void ascii_get_element(PlyFile *plyfile, void *elem_ptr)
  {
	  int j,k;
	  PlyElement *elem;
	  PlyProperty *prop;
	  ...
	  if (elem->other_offset != NO_OTHER_PROPS) {
		  ...
		  memcpy(ptr, &other_data, sizeof(other_data));
	  }
	  else
		  other_flag = 0;
	  ...
	  for (j = 0; j < elem->nprops; j++) {
		  ...
		  if (prop->is_list) {       /* a list */
			  ...
			  get_ascii_item (words[which_word++], prop->count_external,
				  &int_val, &uint_val, &double_val);
			  if (store_it) {
				  item = reinterpret_cast<char *>(elem_data) +
                    prop->count_offset;
			  ...
			  if (list_count == 0) {
                  ...
                  {
                      void *empty(nullptr);
					  memcpy(store_array, &empty, sizeof(empty));
                  }
			  }
			  else {
				  if (store_it) {
					  ...
					  item = item_ptr;
					  memcpy(store_array, &item_ptr, sizeof(item_ptr));
				  }
 ...
 ******************************************************************************/
 ...
 void binary_get_element(PlyFile *plyfile, void *elem_ptr)
 {
	 int j,k;
	 PlyElement *elem;
	 PlyProperty *prop;
...
#define BIG_STRING 4096
  ...
  ******************************************************************************/
  ...
  double get_item_value(void *item, int type)
  {
  ...
  ******************************************************************************/
    template<typename T>
    double writeCast(FILE *fp, void *item, const char *format)
    {
        T t = *reinterpret_cast<T *>(item);
        fprintf(fp, format, t);
  ...
  double old_write_ascii_item(FILE *fp, void *item, int type)
  {
	  ...
	  case PLY_CHAR:
	  case PLY_INT_8:
          return writeCast<char>(fp, item, "%d ");
	  case PLY_UCHAR:
	  case PLY_UINT_8:
          return writeCast<unsigned char>(fp, item, "%d ");
	  case PLY_SHORT:
	  case PLY_INT_16:
          return writeCast<short>(fp, item, "%d ");
	  case PLY_USHORT:
	  case PLY_UINT_16:
          return writeCast<unsigned short>(fp, item, "%d ");
	  case PLY_INT:
	  case PLY_INT_32:
          return writeCast<int>(fp, item, "%d ");
	  case PLY_UINT:
	  case PLY_UINT_32:
          return writeCast<int>(fp, item, "%u ");
	  case PLY_FLOAT:
	  case PLY_FLOAT_32:
          return writeCast<float>(fp, item, "%g ");
	  case PLY_DOUBLE:
	  case PLY_FLOAT_64:
          return writeCast<double>(fp, item, "%g ");
	  default:
  ...
  ******************************************************************************/
  ...
  template<typename T>
  void castAssign(void *loc, const T t)
  {
      *reinterpret_cast<T *>(loc) = t;
  }
	  ...
	  )
  {
	  ...
	  case PLY_CHAR:
	  case PLY_INT_8:
          castAssign(item, (char)int_val);
		  break;
	  case PLY_UCHAR:
	  case PLY_UINT_8:
          castAssign(item, (unsigned char)uint_val);
		  break;
	  case PLY_SHORT:
	  case PLY_INT_16:
          castAssign(item, (short)int_val);
		  break;
	  case PLY_USHORT:
	  case PLY_UINT_16:
          castAssign(item, (unsigned short)uint_val);
		  break;
	  case PLY_INT:
	  case PLY_INT_32:
          castAssign(item, int_val);
		  break;
	  case PLY_UINT:
	  case PLY_UINT_32:
          castAssign(item, uint_val);
		  break;
	  case PLY_FLOAT:
	  case PLY_FLOAT_32:
          castAssign(item, (float)double_val);
		  break;
	  case PLY_DOUBLE:
	  case PLY_FLOAT_64:
          castAssign(item, double_val);
		  break;
	  default:
