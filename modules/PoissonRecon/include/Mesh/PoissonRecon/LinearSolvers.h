...
#include "Mesh/PoissonRecon/SparseMatrixInterface.h"
...
struct CGScratch
{
  ...
  CGScratch(int dim) : r(NULL), d(NULL), q(NULL)
  {
    resize(dim);
  }
  ~CGScratch(void)
...
struct PreconditionedCGScratch : public CGScratch<Real>
{
  ...
  PreconditionedCGScratch(int dim) : CGScratch<Real>()
  {
    resize(dim);
  }
  ~PreconditionedCGScratch(void)
  {
    resize(0);
  }
  void resize(int dim)
                          ...
                          bool verbose                           = false)
{
  ...
  if (scratch)
    ...
    r = new Real[dim], d = new Real[dim], q = new Real[dim], s = new Real[dim];
  memset(r, 0, sizeof(Real) * dim), memset(d, 0, sizeof(Real) * dim),
...
class EigenSolverCholeskyLLt : public EigenSolver<Real, MatrixRowIterator>
{
 ...
 public:
      ...
      bool analyzeOnly = false)
  {
    ...
    for (int i = 0; i < M.rows(); i++)
      for (MatrixRowIterator iter = M.begin(i); iter != M.end(i); iter++)
        triplets.push_back(Eigen::Triplet<double>(i, iter->N, iter->Value));
#ifdef STORE_EIGEN_MATRIX
    ...
    if (!analyzeOnly)
    {
      ...
      if (_solver.info() != Eigen::Success)
        fprintf(stderr,
                "[ERROR] EigenSolverCholeskyLLt::EigenSolverCholeskyLLt Failed "
                "to factorize matrix\n"),
            exit(0);
    }
...
#pragma omp parallel for
    ...
    switch (_solver.info())
    {
      ...
      case Eigen::NumericalIssue:
        fprintf(stderr,
                "[ERROR] EigenSolverCholeskyLLt::update Failed to factorize "
                "matrix (numerical issue)\n"),
            exit(0);
      case Eigen::NoConvergence:
        fprintf(stderr,
                "[ERROR] EigenSolverCholeskyLLt::update Failed to factorize "
                "matrix (no convergence)\n"),
            exit(0);
      case Eigen::InvalidInput:
        fprintf(stderr,
                "[ERROR] EigenSolverCholeskyLLt::update Failed to factorize "
                "matrix (invalid input)\n"),
            exit(0);
      default:
        fprintf(stderr,
                "[ERROR] EigenSolverCholeskyLLt::update Failed to factorize "
 ...
 public:
      ...
      bool analyzeOnly = false)
  {
    ...
    if (!analyzeOnly)
    {
      ...
      if (_solver.info() != Eigen::Success)
        fprintf(stderr,
                "[ERROR] EigenSolverCholeskyLDLt::EigenSolverCholeskyLDLt "
  ...
  void update(const SparseMatrixInterface<Real, MatrixRowIterator>& M)
  {
    ...
    if (_solver.info() != Eigen::Success)
      fprintf(stderr,
              "[ERROR] EigenSolverCholeskyLDLt::update Failed to factorize "
              "matrix\n"),
          exit(0);
  }
  void solve(ConstPointer(Real) b, Pointer(Real) x)
 ...
 public:
                ...
                double tolerance = 0.)
  {
    ...
    if (_solver.info() != Eigen::Success)
      fprintf(
          stderr,
          "[ERROR] EigenSolverCG::EigenSolverCG Failed to factorize matrix\n"),
          exit(0);
...
#pragma omp parallel for
    ...
    if (_solver.info() != Eigen::Success)
      fprintf(stderr,
              "[ERROR] EigenSolverCG::update Failed to factorize matrix\n"),
          exit(0);
  }
...
CholmodSolver::CholmodSolver(
    const SparseMatrixInterface<Real, MatrixRowIterator>& M, bool analyzeOnly)
{
  _init(M);
    ...
    const SparseMatrixInterface<Real, MatrixRowIterator>& M)
{
  ...
  if (LOWER_TRIANGULAR)
  {
    ...
    cholmod_M  = CHOLMOD(allocate_sparse)(
        dim, dim, maxEntries, 0, 1, -1, CHOLMOD_REAL, &cholmod_C);
  }
  else
  {
    maxEntries = (int)M.entries();
    cholmod_M  = CHOLMOD(allocate_sparse)(
        dim, dim, maxEntries, 0, 1, 0, CHOLMOD_REAL, &cholmod_C);
  }
...
#pragma omp parallel for
  for (int i = 0; i < M.rows(); i++)
  {
    int off = (int)_p[i];
    for (MatrixRowIterator iter = M.begin(i); iter != M.end(i); iter++)
      if (!LOWER_TRIANGULAR || iter->N >= i) _x[off++] = double(iter->Value);
  }
  ...
  {
    fprintf(stderr,
            "[WARNING] CholmodSolver::update: Matrix not positive-definite\n");
    return false;
  }
