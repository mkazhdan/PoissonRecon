...
struct MatrixEntry
{
  ...
  MatrixEntry(IndexType i)
  {
    N = i, Value = 0;
  }
  MatrixEntry(IndexType n, T v)
  {
    N = n, Value = v;
  }
  IndexType N;
...
class SparseMatrixInterface
{
 public:
  ...
  void multiplyScaled(T scale,
                      ...
                      int multiplyFlag = 0) const;
  template <class T2>
  void multiply(Pointer(T2) In, Pointer(T2) Out, int multiplyFlag = 0) const
  {
  ...
  void gsIteration(ConstPointer(T) diagonal,
                   ...
                   bool dReciprocal) const;
  template <class T2>
  void gsIteration(const std::vector<int>& multiColorIndices,
                   ConstPointer(T) diagonal,
                   ConstPointer(T2) b,
                   Pointer(T2) x,
                   bool dReciprocal) const;
  template <class T2>
  void gsIteration(const std::vector<std::vector<int> >& multiColorIndices,
                   ConstPointer(T) diagonal,
                   ConstPointer(T2) b,
                   Pointer(T2) x,
                   bool forward,
...
#include "Mesh/PoissonRecon/SparseMatrixInterface.inl"
