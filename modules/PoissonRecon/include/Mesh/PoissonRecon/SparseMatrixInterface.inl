...
template <class T, class const_iterator>
template <class T2>
void SparseMatrixInterface<T, const_iterator>::multiply(ConstPointer(T2) In,
                                                        Pointer(T2) Out,
                                                        int multiplyFlag) const
{
...
void SparseMatrixInterface<T, const_iterator>::multiplyScaled(
    T scale, ConstPointer(T2) In, Pointer(T2) Out, int multiplyFlag) const
{
  ConstPointer(T2) in = In;
#pragma omp parallel for
  for (int i = 0; i < rows(); i++)
...
template <class T, class const_iterator>
void SparseMatrixInterface<T, const_iterator>::setDiagonal(Pointer(T)
                                                               diagonal) const
{
#pragma omp parallel for
  for (int i = 0; i < rows(); i++)
...
template <class T, class const_iterator>
void SparseMatrixInterface<T, const_iterator>::setDiagonalR(Pointer(T)
                                                                diagonal) const
{
#pragma omp parallel for
  for (int i = 0; i < rows(); i++)
...
template <class T2>
void SparseMatrixInterface<T, const_iterator>::gsIteration(
    const std::vector<int>& multiColorIndices,
    ConstPointer(T) diagonal,
    ConstPointer(T2) b,
    Pointer(T2) x,
    bool dReciprocal) const
...
template <class T2>
void SparseMatrixInterface<T, const_iterator>::gsIteration(
    const std::vector<std::vector<int> >& multiColorIndices,
    ConstPointer(T) diagonal,
    ConstPointer(T2) b,
    Pointer(T2) x,
    bool forward,
...
#define ITERATE(indices)                                         \
  {                                                              \
    ...
    if (forward)
      for (int j = 0; j < multiColorIndices.size(); j++)
      {
        ITERATE(multiColorIndices[j]);
      }
    else
      for (int j = int(multiColorIndices.size()) - 1; j >= 0; j--)
      {
        ITERATE(multiColorIndices[j]);
      }
#undef ITERATE
...
int SolveCG(const SPDFunctor& M,
            int dim,
            ConstPointer(T) b,
            int iters,
            Pointer(T) x,
...
#pragma omp parallel for reduction(+ : delta_new)
  for (int i = 0; i < dim; i++)
    d[i] = r[i] = b[i] - r[i], delta_new += Dot(r[i], r[i]);
...
#pragma omp parallel for reduction(+ : delta_new)
      for (int i = 0; i < dim; i++)
        r[i] = b[i] - r[i], delta_new += Dot(r[i], r[i]),
        x[i] += (T)(d[i] * alpha);
    }
    else
#pragma omp parallel for reduction(+ : delta_new)
      for (int i = 0; i < dim; i++)
        r[i] -= (T)(q[i] * alpha), delta_new += Dot(r[i], r[i]),
            x[i] += (T)(d[i] * alpha);
...
#pragma omp parallel for reduction(+ : delta_new)
  for (int i = 0; i < dim; i++)
    d[i] = r[i] = Pb[i] - r[i], delta_new += Dot(r[i], r[i]);
...
#pragma omp parallel for reduction(+ : delta_new)
      for (int i = 0; i < dim; i++)
        r[i] = Pb[i] - r[i], delta_new += Dot(r[i], r[i]),
        x[i] += (T)(d[i] * alpha);
    }
    else
#pragma omp parallel for reduction(+ : delta_new)
